	...
	$.ajax({
		...
		complete: function(xhr){
			...
			if(jsonResult.success===true){
				showMsgPanel('保存成功','window.location="ledgerChrgList?PRD_ID='+selRowId+'"');
			}else{
...
//查找当前是否有选中的记录
	...
	$.ajax({
		...
		type:"POST",
		dataType:"text",
		data:{"PRD_LEDGER_IDS":ids,"PRD_ID":selRowId},
		complete: function(xhr){
			eval("jsonResult = "+xhr.responseText);
			if(jsonResult.success===true){
...
function addRow(arrs){
	...
	$("#jsontb tr:last-child").after("<tr class='list_row"+classrow+"' onmouseover='mover(this)' onmouseout='mout(this)'></tr>");
		    ...
		    html+="<td nowrap align='center'><input type='checkbox' style='height:12px;valign:middle' style='width:100%' json='PRD_LEDGER_ID' name='PRD_LEDGER_ID"+rownum+"' value='"+arrs[0]+"'/></td>"+
...
function gobacknew(){
	var channId = getSelRowId();
	 window.location="ledgerChrgList?PRD_ID="+channId; 
}
