...
//		   showAuditDiv();
		   ...
		   var selRowId = $("#selRowId").val();
			parent.window.myShowModalDialog("toAudit?depositId="+selRowId,true, 1000);
		});
});	
...
function auditCommon(type,remark){
	...
	var index = parent.layer.load(1);
	 $.ajax({
	 	url: "toAudit?depositId="+selRowId+"&remark="+remark+"&type="+type,
		type:"POST",
...
function listAuditConfirm(){
	...
	var selRowId = $("#selRowId").val();
	 $.ajax({
		url: "toAudit?depositId="+utf8(selRowId),
		type:"POST",
		dataType:"text",
		complete: function(xhr){
			eval("jsonResult = "+xhr.responseText);
...
function listStartConfirm(){
	...
	var selRowId = $("#selRowId").val();
    $.ajax({
	 url: "toCommit?depositId="+utf8(selRowId),
	 type:"POST",
 	 dataType:"text",
	 complete: function(xhr){
		eval("jsonResult = "+xhr.responseText);
