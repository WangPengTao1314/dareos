...
$(function(){
	...
	//审核页面需要隐藏的按钮
	shBtnHidden(["addBtn", "modifyBtn","deleteBtn", "commitBtn", "revokeBtn", "confirmquoteBtn","shouhouBtn"]);
	//主从及主从从列表页面通用加载方法
...
//		toFlow("3");
	...
	// 销售订单退回到报价中的预订单
	$("#saleBack").click(function() {
		var selRowId = parent.document.getElementById("selRowId").value;
		if (selRowId == "") {
			parent.showWarnMsg("\u8bf7\u9009\u62e9\u4e00\u6761\u6570\u636e");
		}else{
			doConfirm(selRowId, '强制退回并删除销售订单', 'saleBack', '强制退回并删除销售订单');
		}
	});
	$("#flow").click(function () {
		var selRowId = parent.document.getElementById("selRowId").value;
	...
	$("#export").click(function(){
		...
		window.location.href = "exportExcelDtl?GOODS_ORDER_ID="+selRowId;
	});
...
function doConfirm(selRowId, title, func, reason){
	layer.confirm('是否确认'+title+'？', {
		...
		$.ajax({
			...
			type:"POST",
			dataType:"text",
			data:{"SALE_ORDER_ID": selRowId, reason: reason},
			complete: function (xhr){
				layer.close(index);
				eval("jsonResult = "+xhr.responseText);
				if(jsonResult.success===true){
...
function setBtnState(state) {
	...
	//当状态=草稿
	if (state == "草稿" || state == "退回") {
		btnDisable(["revoke","confirmquote","shouhou","audit","quote","confirm"]);
	}
	...
	if (state == "待处理" || state == "已转销售订单") {
		btnDisable(["modify","delete","commit","revoke","confirmquote","shouhou","audit","quote","back", "confirm","modifyHandle"]);
	}
