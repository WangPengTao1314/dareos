...
**/
	...
	$('#back2c').click(function(){
		// 一级经销商退回订货订单
		var RETURN_RSON = $('#auditContents').val();
		if (RETURN_RSON.length == 0) {
		...
		layer.confirm('是否确定退回？', {
				...
				$.ajax({
					...
					async : false,
					dataType:"json",
					data:{"selRowId": selRowId, "RETURN_RSON": RETURN_RSON},
	...
	// 审图、报价
	$('#tmpsave,#audit,#unaudit').click(function(){
		var selRowId = getSelRowId();
	...
	$('#confirm,#unconfirm').click(function(){
		var selRowId = getSelRowId();
		var option = $('#option').val();
		var status = $(this).attr('status');
		$('#auditStatus').val(status);
...
function allSave(func) {
	...
	//$("#jsontb tr:gt(0) input[json=PRD_ID]").not(":empty").parent().parent().find("input[json=GOODS_ORDER_DTL_ID]").attr('checked', 'checked');
	var childData;
...
function init(){
	var option = $('#option').val();
	if (option && 'audit' == option){//审图
		$('#mainForm img').remove();
		$('#mainForm :input').not(':button, :submit, :reset, :hidden, .audit').attr('disabled', 'disabled');
		$('#mainForm :input').not(':button, :submit, :reset, :hidden, textarea').css("width", "90%");
		$('#mainForm .gdtd_tb_textarta').css("width", "95%");
		//$('#mainForm #attPath').removeAttr('disabled');
		$('.child img').remove();
		$('.child :input').not(':button, :submit, :reset, :hidden').attr('disabled', 'disabled');
		//$('.child input[json=GOODS_ORDER_DTL_ID]').removeAttr('disabled');
	} else if (option && 'quote' == option){//报价
		$('#mainForm img').not('.quote').remove();
		$('#mainForm :input').not(':button, :submit, :reset, :hidden, .audit, .quote').attr('disabled', 'disabled');
		//$('#mainForm :input').not(':button, :submit, :reset, :hidden, textarea, .quote').css("width", "90%");
		//$('#mainForm .gdtd_tb_textarta').css("width", "95%");
		// 附件添加只读属性
		$('#mainForm #attPath').attr('disabled', 'disabled');
		$('#quoteTd').show();
		$('.child input[json=attPath]').attr('disabled', 'disabled');
	} else if (option && 'confirmquote' == option){//经销商确认报价，所有元素只读
		$('#mainForm img').remove();
		$('#mainForm :input').not(':button, :submit, :reset, :hidden, .audit').attr('disabled', 'disabled');
		$('#mainForm :input').not(':button, :submit, :reset, :hidden, textarea').css("width", "90%");
		$('#mainForm .gdtd_tb_textarta').css("width", "95%");
		...
		$('.child input[json=attPath]').attr('disabled', 'disabled');
		//$('.child input[json=GOODS_ORDER_DTL_ID]').removeAttr('disabled');
	} else if (option && 'confirm' == option){//总部领导审核
		$('#mainForm img').remove();
		$('#mainForm :input').not(':button, :submit, :reset, :hidden, .audit').attr('disabled', 'disabled');
		$('#mainForm :input').not(':button, :submit, :reset, :hidden, textarea').css("width", "90%");
		$('#mainForm .gdtd_tb_textarta').css("width", "95%");
		...
		$('#quoteTd').show();
		$('#quoteTd :input').attr('disabled', 'disabled');
		$('.child img').not('.quote').remove();
		// 附件添加只读属性
		$('.child input[json=attPath]').attr('disabled', 'disabled');
	} else if (option && 'pricing'==option){//一级经销商核价
		$('#mainForm img').remove();
		$('#mainForm :input').not(':button, :submit, :reset, :hidden, .pricing').attr('disabled', 'disabled');
		// 附件添加只读属性
		$('#mainForm #attPath').attr('disabled', 'disabled');
		$('#quoteTd').show();
		$('.child input[json=attPath]').attr('disabled', 'disabled');
...
// 选择经销商(一级经销商选择自己及其下级)
function selChann() {
	var CHANN_ID = $('#SALE_ID').val();
	var initSel = " STATE = '启用' and IS_BASE_FLAG = '0' and ( CHANN_ID = '" + CHANN_ID + "' or CHANN_ID_P='" + CHANN_ID + "') ";
	$('#initSel').val(initSel);
	selCommon("BS_19", false, "RECV_CHANN_ID", "CHANN_ID", "forms[0]",
			"RECV_CHANN_NO,RECV_CHANN_NAME,PRINT_NAME",
			"CHANN_NO,CHANN_ABBR,CHANN_ABBR", "initSel");
...
function addRow(arrs) {
	...
	soDtl += '<td nowrap><input type="text" id="PRD_NO' + rownum + '" json="PRD_NO" size="10" name="PRD_NO" value="' + arrs[2] + '" title="' + arrs[2] + '" label="产品编码" autocheck="true" mustinput="true" inputtype="string" readonly style="padding-right: 20px;" />'
						+ '<img name="imgTab" align="absmiddle" class="magnifier_mx" src="' + ctxPath + '/styles/newTheme/images/plus/select.gif" onClick=\'selPrd(' + rownum + ')\'/>'
	...
	soDtl += '<td nowrap class="SHOWPRICE" ><input type="text" class="decimal quote pricing" id="PRICE' + rownum + '" json="PRICE" size="5" name="PRICE" value="' + arrs[17] + '" '
					+'label="计算报价" autocheck="" mustinput="" inputtype="zfloat" '
	...
	soDtl += '<td nowrap class="SHOWPRICE" ><input type="text" class="decimal quote" id="REBATE_AMOUNT' + rownum + '" json="REBATE_AMOUNT" size="5" name="REBATE_AMOUNT" value="' + arrs[25] + '" '
					...
					+'readonly /></td>'; // 返利
	soDtl += '<td nowrap class="SHOWPRICE" ><input type="text" class="decimal quote" id="DECT_PRICE' + rownum + '" json="DECT_PRICE" size="5" name="DECT_PRICE" value="' + arrs[19] + '" '
					+'label="最终报价" autocheck="" mustinput="" inputtype="zfloat" valueType="10,2" '
					+'onkeyup="changeDectPrice(' + rownum + ')" /></td>'; // 最终报价
	soDtl += '<td nowrap class="SHOWPRICE" ><input type="text" class="decimal quote" id="ORDER_AMOUNT' + rownum + '" json="ORDER_AMOUNT" size="6" name="ORDER_AMOUNT" value="' + arrs[21] + '" '
					+'label="金额" autocheck="" mustinput="" inputtype="zfloat" valueType="10,2" '
					+'onkeyup="changeAmount(' + rownum + ')" /></td>'; // 总金额
	soDtl += '<td nowrap><textarea class="" id="REMARK' + rownum + '" json="REMARK" name="REMARK" inputtype="string" title="' + arrs[22] + '" >' + arrs[22].replaceAll('；', '&#13;') + '</textarea></td>';
	soDtl += '</tr>';
	...
	var option = $('#option').val();
	if (option && ('audit' == option || 'quote' == option || 'confirmquote' == option || 'confirm' == option || 'pricing' == option)){
		...
		if ('quote' == option || 'pricing' == option) {
			...
			$("#jsontb tr:last-child").find("input[json=DECT_PRICE]").attr("autocheck","true");
			$("#jsontb tr:last-child").find("input[json=DECT_PRICE]").attr("mustinput","true");
			$("#jsontb tr:last-child").find("input[json=ORDER_AMOUNT]").attr("autocheck","true");
			$("#jsontb tr:last-child").find("input[json=ORDER_AMOUNT]").attr("mustinput","true");
			trCheckInit($("#jsontb tr:last-child :input"));
		} else if ('confirm' == option) {
			$('#jsontb tr:last-child img').not('.quote').remove();
			$('#jsontb tr:last-child :input').not(':button, :submit, :reset, :hidden, .edit').attr('disabled', 'disabled');
			trCheckInit($("#jsontb tr:last-child :input"));
		} else {
	...
	} else if (option && 'confirmquote' == option){
		$('#jsontb tr:last-child img').remove();
		$('#jsontb tr:last-child :input').not(':button, :submit, :reset, :hidden').attr('disabled', 'disabled');
		$("#jsontb tr:last-child").find('#attPath'+ rownum).attr('disabled', 'disabled');
...
function changeDectPrice(rownum) {
	var DECT_PRICE = $("#DECT_PRICE" + rownum).val(); // 最终报价
	DECT_PRICE = isNaN(parseFloat(DECT_PRICE)) ? 0 : parseFloat(DECT_PRICE);
	var temp_ORDER_NUM = $("#ORDER_NUM" + rownum).val();// 数量
	var ORDER_NUM = isNaN(temp_ORDER_NUM) ? 0 : parseFloat(temp_ORDER_NUM); // 数量
	// 总金额，保留2位小数
	var ORDER_AMOUNT = Math.round((isNaN(ORDER_NUM * DECT_PRICE) ? 0
	...
	var REBATE_AMOUNT = $("#REBATE_AMOUNT" + rownum).val();// 返利
	var rebate_amount_pre = Number(ORDER_AMOUNT)+Number(REBATE_AMOUNT);
	// 折扣率
	var danjia = $("#PRICE" + rownum).val();
	var price = isNaN(parseFloat(danjia)) ? 0 : parseFloat(danjia); // 单价
	if(price == 0){
		parent.showWarnMsg("请先输入计算报价");
		$("#DECT_RATE" + rownum).val("100");
		return;
	}
	var zhekoulv = Math.round(isNaN(rebate_amount_pre / ORDER_NUM / price) ? 0
			: rebate_amount_pre / ORDER_NUM / price * 100 * 100) / 100;
	// 判断输入是否为数字，如果不是数字则默认为100
	var dect_rate = isNaN(parseFloat(zhekoulv)) ? 100 : parseFloat(zhekoulv); // 折扣率
	dect_rate = zhekoulv == 0 ? 100 : zhekoulv; // 折扣率
	$("#DECT_RATE" + rownum).val(dect_rate);
...
function changeAmount(rownum) {
	var ORDER_AMOUNT = $("#ORDER_AMOUNT" + rownum).val();// 金额
	var REBATE_AMOUNT = $("#REBATE_AMOUNT" + rownum).val();// 返利
	var rebate_amount_pre = Number(ORDER_AMOUNT)+Number(REBATE_AMOUNT);
	var temp_ORDER_NUM = $("#ORDER_NUM" + rownum).val();// 数量
	var ORDER_NUM = isNaN(temp_ORDER_NUM) ? 0 : parseFloat(temp_ORDER_NUM); // 数量
