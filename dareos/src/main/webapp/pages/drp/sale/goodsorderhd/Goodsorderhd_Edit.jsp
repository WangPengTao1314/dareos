...
<head>
	...
	<title id="title_text">
		...
		<c:if test="${'pricing' eq option }">报价</c:if>
	</title><%-- 
	<script src="${ctx }/scripts/layui/layui.js" charset="utf-8"></script>
   ...
   <!--  <span class="add_demospan">当前位置：经销商>订货单管理</span> -->
		<table id="jsontbP" width="100%" border="0" cellpadding="0" cellspacing="0" class="detail">
			...
			<tr>
				...
				<!-- 流程退回测试input type="hidden" id="FLOW_ID" /-->
					<input type="hidden" id="DELIVER_ADDR_ID" />
					<input type="hidden" id="initSel" value=" CHANN_ID = '${rst.CHANN_ID }' "/>
					...
					<table cellspacing="0" cellpadding="0" class="detail3"
						...
						<tr>
							<td class="gdtd">
							<div>销售类型：</div>
							<select class="gdtd_select_input quote" json="BILL_TYPE" name="BILL_TYPE" id="BILL_TYPE"
						...
						<tr class="yhzz">
							...
							<td  class="gdtd">
								<div>经销商名称：</div>
								<input class="gdtd_select_input" type="text" value="${rst.CHANN_NAME}" 
						...
						<tr class="yhzz">
							...
							<td >
								<div>下拉选择框</div>
							      <select name="city" lay-filter="city" lay-verify="" lay-search class="gdtd_select_input">
						...
						<tr class="ysfs">
							<td  class="gdtd">
								...
								<input class="gdtd_select_input" type="text" value="${rst.CUST_NAME}" json="CUST_NAME"
									...
									label="客户姓名" autocheck="true" inputtype="string" mustinput="true" maxlength="200" />
							</td>
							<td class="gdtd">
								<div>客户电话：</div>
							...
							<td class="gdtd" colspan="2">
								...
								<input class="gdtd_select_input" type="text" value="${rst.CUST_ADDR}" json="CUST_ADDR"
									...
									label="客户住址" autocheck="true" inputtype="string" mustinput="true" maxlength="250" />
							</td>
						</tr>
						...
						<tr>
							<td nowrap class="gdtd" colspan="2">
								...
								<textarea style="width:92.5%; " class="audit" json="auditContents" name="auditContents" id="auditContents" inputtype="string"
		...
		<table id="qxBtnTb" cellpadding="0" cellspacing="0" border="0" style=" float: right; ">
			<tr style="height: 40px">
				...
				<td nowrap style="word-spacing: 6px;">
					<c:if test="${empty option or 'pricing' eq option or 'turnToHQ' eq option or 'quote' eq option}">
		...
		<c:if test="${not empty option }">
			...
			<button class="img_input">
				<label for="unaudit">
					<img src="${ctx}/styles/${theme}/images/icon/tuihui.png"
						class="icon_font">
					<input id="unaudit" type="button" class="btn" value="退回" status="F">
				</label>
			</button>
			</c:if>
			...
			<button class="img_input">
				<label for="unconfirm">
					<img src="${ctx}/styles/${theme}/images/icon/tuihui.png"
						class="icon_font">
					<input id="unconfirm" type="button" class="btn" value="退回" status="F">
				</label>
			</button>
			<!-- 流程退回测试button class="img_input">
				<label for="ceshi">
					<img src="${ctx}/styles/${theme}/images/icon/tuihui.png"
						class="icon_font">
					<input id="ceshi" type="button" class="btn" value="退回测试" >
				</label>
			</button-->
			</c:if>
			<c:if test="${'turnToHQ' ne option}">
			</c:if>
		</c:if>
		<c:if test="${'turnToHQ' eq option}">
			<button class="img_input" >
				<label for='back2c'>
					<img src="${ctx}/styles/${theme}/images/icon/tuihui.png"  class="icon_font">
					<input id="back2c" type="button" class="btn" value="退回" />
				</label>
			</button>
			<button class="img_input">
				<label for="save">
					<img src="${ctx}/styles/${theme}/images/icon/tijiao.png"
						class="icon_font">
					<input id="save" type="button" class="btn" value="提交">
				</label>
			</button>
		</c:if>
		<c:if test="${not empty option and 'pricing' eq option}">
		<button class="img_input">
			<label for="save">
				<img src="${ctx}/styles/${theme}/images/icon/baocun.png"
					class="icon_font">
