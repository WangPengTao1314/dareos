				...
				<td class="detail2">
					<table cellspacing="0" cellpadding="0" width="98%" height="98%" id="myTable"  class="detail3"
						...
						<tr>
							...
							<td class="gdtd_tb">
								<div>订单图纸&nbsp;：
									<button type="button" class="layui-btn uploadFile"  id="ATT_PATH" lay-data="{id:'ATT_PATH'}" disabled="true">上传</button>
								</div> 
						...
						<tr>
							<td colspan="8">
								<div>备注：</div>
								<textarea style="width: 45.5%;resize:none" 
								rows="5"	READONLY>${entry.REMARK}</textarea>
							</td>
							<td class="gdtd_tb"></td>
							<td class="gdtd_tb"></td>
							<td class="gdtd_tb"></td>
						...
						<tr>
							...
							<td class="gdtd_tb">
								<div>收货组织：</div>
								<textarea class="gdtd_select_input"  READONLY>${entry.RECV_CHANN_NAME}</textarea>
							</td>
							<td class="gdtd_tb">
								<div>经销商名称：</div>
						...
						<tr>
							...
							<td class="gdtd_tb">
								<div>客户电话：</div>
								<textarea class="gdtd_select_input"   READONLY>${entry.CUST_TEL}</textarea>
							</td>
							<td class="gdtd_tb">
								<div>客户住址：</div>
						...
						<tr>
							<td>
								<div>备注：</div>
								<textarea style="width: 188.5%;" rows="5"  READONLY>${entry.REMARK2}</textarea>
							</td>
							<td class="gdtd_tb"></td>
							<td class="gdtd_tb"></td>
							<td class="gdtd_tb"></td>
					...
					<table id="jsontb" width="100%" border="0" cellpadding="5" cellspacing="0" class="lst" >
						...
						<c:forEach items="${entry.entrySun}" var="rst" varStatus="row">
							...
							<tr class="list_row${r}" onmouseover="mover(this)" onmouseout="mout(this)"  id="tr${rr}">
								...
								<td nowrap class="LMM">
									<c:if test="${rst.IS_NO_LOCK_FLAG eq 0}">否</c:if>
									<c:if test="${rst.IS_NO_LOCK_FLAG eq 1}">是</c:if>
								</td>
								<td nowrap>${rst.FIGURE_NO}</td>
								<td nowrap align="center">
										<div style="float:left">
											<input type="hidden" id="hid_ATT_PATH${row.index }"  json="ATT_PATH" name="ATT_PATH" value="${ rst.ATT_PATH}" />
											<button type="button" class="layui-dtlbtn " id="ATT_PATH${row.index }" lay-data="{fileid:\'/a/\'}" title="上传附件" disabled >
											<img src="${ctx}/styles/${theme}/images/icon/upload.png" ></button>
										</div>
										<div id="view_ATT_PATH${row.index }"></div>
								</td>
								<td nowrap>
									<c:if test="${rst.IS_NO_REBATE_FLAG eq 0}">否</c:if>
									<c:if test="${rst.IS_NO_REBATE_FLAG eq 1}">是</c:if>
								</td>
...
var interval=setInterval(function(){
	if(uploadFile){
		...
		<c:forEach items="${entry.entrySun}" var="rst_child" varStatus="row">
			//上传文件
			uploadFile('ATT_PATH${row.index }','',true,false,true,false,true);
		</c:forEach>
	}
},500);
