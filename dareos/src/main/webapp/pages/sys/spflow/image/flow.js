...
function Node(nID,nName,nType,nX,nY,operaName)
{
...
function createFlow() {
	...
	for (var i=0;i<nodeLength;i++)
	{
	   ...
	   var tempNode=new Node(id[i],name[i],type[i],iX[i],iY[i],operaName[i]);
...
function drawNode(cNode) {
  ...
  //tempNode.style.backgroundColor = "#0000ff";
  //---添加tempNode
...
function drawPath(startNode,endNode) {
	...
	var endX=0;
	var endY=0;
    endX=Number(endNode.sx)+Number(NodeW)/2;
	endY=endNode.sy;
	startX=Number(startNode.sx)+Number(NodeW)/2;
	startY=Number(startNode.sy)+Number(NodeH);
	//startY=startNode.sy;
  //alert("sX:"+startX+"sY"+startY+"ex"+endX+"eY"+endY);
	if (Number(endNode.n_type)==-3)
	{
		endY=Number(endNode.sy)+Number(NodeH)/2;
	}else {
	drawAcute(endX,endY);
	}
	if (Number(startNode.n_type)==-3)
	{
		startY=Number(startNode.sy)+Number(NodeH)/2; 
	}
...
{ //alert(6);
  if (Number(window.event.button)!=2)
  {
...
{
var mouseX=Number(window.event.clientX) + PX;
var mouseY=Number(window.event.clientY) + PY;
//---判断当前鼠标位置
...
if (curElement!=null)
  ...
  //拖动条跟随
  fake.style.posLeft = Number(mouseX)-15;
  fake.style.posTop = Number(mouseY)-15;
  fake.style.display = '';

  //--根据当前位置，改变拖动条的文字。及拖动条表格变色
	if (inWhoTop)
	{
		fakeNodeTable.style.backgroundColor="#00FF00";
	...
	{
		fakeNodeTable.style.backgroundColor="#FF0000"
...
if (curElement!=null)
  ...
  if (inWhoTop!=null||inWhoBottom!=null||inWhoLeft!=null||inWhoRight!=null)
  {
      ...
      var choose=window.showModalDialog('/sleemon/system/flow.shtml?action=toADDMODNODE&event=SelectNodeEvent&OperType='+fakeType+'&BusinessID='+submitform.ProcessClass.value,'','dialogwidth=450px; dialogheight=260px; status=no');
      //--如果返回结果不为空的话，提交节点
	 if (choose!=null) {
			window.status="";
			...
			submitform.CancleFlag.value = choose[4];
			submitform.AgentFlag.value = choose[6];
			submitform.DelayFlag.value = choose[9];
			submitform.AllFlag.value = choose[7];      
			submitform.DelayDay.value = choose[10];
			submitform.nodeOperationId.value = choose[11];
			submitform.OperType.value = fakeType;
			if(submitform.event.value=='seriesUp')
			{
			 submitform.action="/sleemon/system/flow.shtml?action=addMODNODEUP";
			}else if(submitform.event.value=='seriesDown')
			{
			 submitform.action="/sleemon/system/flow.shtml?action=addMODNODEDOWN";
			}else if(submitform.event.value=='parallelLeft')
			{
			 submitform.action="/sleemon/system/flow.shtml?action=addMODNODELEFT";
			}else if(submitform.event.value=='parallelRight')
			{
			 submitform.action="/sleemon/system/flow.shtml?action=addMODNODELEFT";
...
if (moveElement!=null)
{
  ...
  {
  //---获得当前位置，提交
  var mouseX=Number(window.event.clientX) + Number(PX)-15;
  var mouseY=Number(window.event.clientY) + Number(PY)-15;
...
function doscroll(){
      ...
      folder.style.top  = PY;
      folder.style.left = Number(PX) + Number(foldervar);
      hotdiv.style.top  = PY;
...
function judgeMousePosition(cX,cY) {
   ...
   {
      //----虚节点不判断
      if (Number(sFlow.sNode[i].n_type)!=-3) {
      //---得到节点的中心位置
	  nodeX=Number(sFlow.sNode[i].sx)+Number(halfNodeW);
	  nodeY=Number(sFlow.sNode[i].sy)+Number(halfNodeH);
      	  
 	  
         //--根节点和未节点不判断左右
	  if (Number(sFlow.sNode[i].n_type)!=-1&&Number(sFlow.sNode[i].n_type)!=-2)
      {
       
       //--判断当前位置是否在节点的左边
       if ((cX<=(Number(nodeX)-Number(halfNodeW)))&&(cX>(Number(nodeX)-Number(halfNodeW)-Number(halfNodeZoneW)))&&(cY<(Number(nodeY)+Number(halfNodeH)))&&(cY>(Number(nodeY)-Number(halfNodeH))))
       {
	   ...
	   //--判断当前位置是否在节点的右边
	   if ((cX>=(Number(nodeX)+Number(halfNodeW)))&&(cX<(nodeX+Number(halfNodeW)+Number(halfNodeZoneW)))&&(cY<(nodeY+Number(halfNodeH)))&&(cY>(nodeY-Number(halfNodeH))))
	   {
		   inWhoRight = sFlow.sNode[i];
		   //window.status="当前位置-->"+inWhoRight.n_name+"的右边";
		   break;
	   }
	   if ((cX>(nodeX-Number(halfNodeW)))&&(cX<(nodeX+Number(halfNodeW)))&&(cY>(nodeY-Number(halfNodeH)))&&(cY<(nodeY+Number(halfNodeH))))
	   {
           inWho = sFlow.sNode[i];
	   ...
	   //---根节点不判断上节点
	   if (Number(sFlow.sNode[i].n_type)!=-1)
	   {
	   
	   if ((cX>(nodeX-Number(halfNodeW)))&&(cX<(nodeX+Number(halfNodeW)))&&(cY<=(nodeY-Number(halfNodeH)))&&(cY>(Number(nodeY)-Number(halfNodeH)-Number(halfNodeZoneH))))
	   {
	   ...
	   //--末节点不判断下节点
	   if (Number(sFlow.sNode[i].n_type)!=-2)
	   {
		if ((cX>(nodeX-halfNodeW))&&(cX<(nodeX+Number(halfNodeW)))&&(cY>=(nodeY+Number(halfNodeH)))&&(cY<(nodeY+Number(halfNodeH)+Number(halfNodeZoneH))))
	   {
...
if(conf)
  {
	...
	submitform.action="/sleemon/system/flow.shtml?action=delMODNODE";
    submitform.submit();
  }
chooseNode=null;
}
...
function Node_Edit() {
	...
	//alert(chooseNode.n_id);
	choose = window.showModalDialog('/sleemon/system/flow.shtml?action=toEditNODE&flowNodeId='+chooseNode.n_id+'&BusinessID='+submitform.ProcessClass.value,'','dialogwidth=450px; dialogheight=260px; status=no');
	if (choose!=null) {
	submitform.ProNodeID.value = chooseNode.n_id;
	submitform.OperID.value =  choose[0];
	submitform.BackFlag.value = choose[2];
	...
	submitform.DelayDay.value = choose[10];
	submitform.nodeOperationId.value = choose[11];
	submitform.action="/sleemon/system/flow.shtml?action=toSAVENODE";
	submitform.submit(); 
	} 
	chooseNode=null;
}
